...
class DuoRec(SequentialRecommender):
    ...
    def __init__(self, config, dataset, num_items=None):
        if dataset is not None:
            super(DuoRec, self).__init__(config, dataset)
        elif num_items is not None:
            mock_dataset = _MockDataset(num_items)
            super(DuoRec, self).__init__(config, mock_dataset)
        else:
            raise ValueError("DuoRec: both dataset and num_items is None! Specify at least one.")
