...
class Trainer(AbstractTrainer):
    ...
    def _build_optimizer(self, params):
        ...
        if (
            ...
            and self.weight_decay
            and self.weight_decay * self.config["reg_weight"] > 0
        ):
            self.logger.warning(
        ...
        if self.config["scheduler"] is not None:
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=self.epochs, eta_min=self.learning_rate / 20)
    ...
    def fit(
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                if callback_fn:
                    callback_fn(epoch_idx, valid_score, valid_result, self)
    ...
    def evaluate(
        ...
        if eval_data.dl_type == DataLoaderType.FULL:
            if self.item_tensor is None:
                self.item_tensor = (
                    eval_data.get_item_feature().to(self.device).repeat(eval_data.step)
                )
