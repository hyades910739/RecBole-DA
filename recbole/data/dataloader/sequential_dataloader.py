...
class SequentialDataLoader(AbstractDataLoader):
    ...
    def __init__(
        ...
        super().__init__(
            config, dataset, batch_size=batch_size, dl_format=dl_format, shuffle=shuffle
        )

    def data_preprocess(self):
        """Do data augmentation before training/evaluation."""
        self.pre_processed_data = self.augmentation(
            self.item_list_index, self.target_index, self.item_list_length
        )
        # used for DuoRec semantic positive sampling
        if self.config["model"] == "DuoRec":
            self.static_item_id_list = (
                self.pre_processed_data["item_id_list"].detach().clone()
            )
            self.static_item_length = (
                self.pre_processed_data["item_length"].detach().clone()
            )
    ...
    def cl4srec_aug(self, cur_data):
        ...
        def item_mask(seq, length, gamma=0.3):
            ...
            masked_item_seq[
                mask_index
            ] = self.dataset.item_num  # token 0 has been used for semantic masking
    ...
    def augmentation(self, item_list_index, target_index, item_list_length):
        ...
        """
        # turn previous sub seq to tensor
