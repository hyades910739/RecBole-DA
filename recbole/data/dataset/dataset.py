...
class Dataset(object):
    ...
    def _get_illegal_ids_by_inter_num(
        self, field, feat, inter_num, max_num=None, min_num=None
    ):
    ...
    def _filter_by_field_value(self):
        ...
        filter_field = []
        filter_field += self._drop_by_value(
            self.config["lowest_val"], lambda x, y: x < y
        )
        filter_field += self._drop_by_value(
            self.config["highest_val"], lambda x, y: x > y
        )
        filter_field += self._drop_by_value(
            self.config["equal_val"], lambda x, y: x != y
        )
        filter_field += self._drop_by_value(
            self.config["not_equal_val"], lambda x, y: x == y
        )
    ...
    def _get_fields_in_same_space(self):
        ...
        for field in token_like_fields:
            ...
            if count == 0:
                ...
                raise ValueError(
                    f"Field [{field}] occurred in `fields_in_same_space` more than one time."
                )
    ...
    def _get_remap_list(self, field_set):
        ...
        """
        remap_list = []
        for field, feat in zip(
            [self.uid_field, self.iid_field], [self.user_feat, self.item_feat]
        ):
            if field in field_set:
    ...
    def _remap_ID_all(self):
        ...
        self.logger.debug(
            set_color("fields_in_same_space", "blue") + f": {fields_in_same_space}"
        )
        for field_set in fields_in_same_space:
