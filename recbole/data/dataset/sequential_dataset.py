...
class SequentialDataset(Dataset):
    ...
    def semantic_augmentation(self, target_index):
        ...
        if os.path.exists(aug_path):
            ...
            same_target_index = []
            # target_index : the target item index to each sub-seq
            # target_item : the actual item_id to each target index.
    ...
    def leave_one_out(self, group_by, leave_one_num=1):
        self.logger.debug(
            f"Leave one out, group_by=[{group_by}], leave_one_num=[{leave_one_num}]."
        )
        if group_by is None:
        ...
        if self.config["SSL_AUG"] == "DuoRec":
            # array[list] : array[i] is the indice of seqs whose target item equals array[i]'s.
    ...
    def inter_matrix(self, form="coo", value_field=None):
        ...
        local_inter_feat = self.inter_feat[
            self.mask
        ]  # TODO: self.mask will applied to _history_matrix() in future
    ...
    def build(self, eval_setting):
        ...
        if ordering_args["strategy"] == "shuffle":
            raise ValueError(
                "Ordering strategy `shuffle` is not supported in sequential models."
            )
        elif ordering_args["strategy"] == "by":
            if ordering_args["field"] != self.time_field:
                raise ValueError(
                    "Sequential models require `TO` (time ordering) strategy."
                )
            if ordering_args["ascending"] is not True:
                raise ValueError(
                    "Sequential models require `time_field` to sort in ascending order."
                )
        ...
        if split_args["strategy"] == "loo":
            return self.leave_one_out(
                group_by=group_field, leave_one_num=split_args["leave_one_num"]
            )
        else:
            ValueError(
                "Sequential models require `loo` (leave one out) split strategy."
            )
