...
class EvalSetting(object):
    ...
    def __str__(self):
        ...
        if (
            self.neg_sample_args is not None
            and self.neg_sample_args["strategy"] != "none"
        ):
            info.append(
                set_color("Negative Sampling", "blue") + f": {self.neg_sample_args}"
            )
        else:
    ...
    def set_ordering(self, strategy="none", **kwargs):
        ...
        if strategy not in legal_strategy:
            raise ValueError(
                "Ordering Strategy [{}] should in {}".format(
    ...
    def set_splitting(self, strategy="none", **kwargs):
        ...
        if strategy not in legal_strategy:
            raise ValueError(
                "Split Strategy [{}] should in {}".format(
                    strategy, list(legal_strategy)
                )
            )
    ...
    def set_neg_sampling(self, strategy="none", distribution="uniform", **kwargs):
        ...
        if strategy == "full" and distribution != "uniform":
            raise ValueError(
                "Full Sort can not be sampled by distribution [{}]".format(distribution)
            )
    ...
    def set_ordering_and_splitting(self, es_str):
        ...
        if ordering_args == "RO":
            ...
            raise NotImplementedError(
                f"Ordering args `{ordering_args}` is not implemented."
            )
