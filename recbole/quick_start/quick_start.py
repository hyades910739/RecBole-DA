...
def run_recbole(
    ...
    sns.set_context(
        "notebook", font_scale=1.8, rc={"lines.linewidth": 3, "lines.markersize": 20}
    )
    ...
    # plt.show()
    plt.savefig(
        log_dir + "/" + config["model"] + "-" + config["dataset"] + ".pdf",
        format="pdf",
        transparent=False,
        bbox_inches="tight",
    )
    ...
    # plt.show()
    plt.savefig(
        log_dir + "/svs.pdf", format="pdf", transparent=False, bbox_inches="tight"
    )
    ...
    if callable(evaluate_hook):
        try:
            ...
            logger.warning(f"Failed to run evaluate_hook: {e}")
    return result
